        ...
        <div className="relative">
                ...
                <div className="flex flex-col text-lg font-light space-y-1">
                        ...
                        <label className={`inline-flex items-center cursor-pointer`} key={index}>
                            ...
                            <span className="ml-2 flex justify-between items-center w-full">
                                ...
                                {currentCities(data) > 0 ? (
                                    <span className="italic text-sm font-normal">{currentCities(data)} town(s) selected</span>
                                ) : (
                                    <>{queryDataParent.includes(data.slug) && <span className="italic text-sm font-normal">Click to remove</span>}</>
                                )}
                            </span>
                        </label>
                    ))}
                    ...
                    {props.showCount && (
                        <div className="mt-5 font-medium underline flex items-center cursor-pointer text-custom-primary" onClick={onClickShowMore}>
                            <span className={'mr-4'}>
                                {' '}
                                {showCount === datas?.length ? 'Less More' : 'See ' + (datas.length - showCount) + ' More'}
                            </span>
                        </div>
                    )}
                </div>
            ...
            {selectParent && (
                <div className="bg-white flex flex-col text-lg font-light space-y-1">
                    ...
                    <div className="">
                        <div
                            className={`capitalize cursor-pointer ${queryData.includes(currentParentData[0].slug) ? 'font-normal' : ''}`}
                        ...
                        <div className="pl-3 flex flex-col text-lg font-light space-y-1 mt-3">
                            {currentParentData[0].children.map((data, index) => (
                                <label
                                    className={`inline-flex items-center cursor-pointer ${queryData.includes(data.slug) ? 'font-normal' : ''}`}
                                    key={index}
                                >
                                    <input
                                        type="checkbox"
