...
import { faBookOpen, faChevronDown, faChevronLeft, faChevronRight, faPlay, faPlayCircle, faUniversity } from '@fortawesome/free-solid-svg-icons';
                        ...
                        <div className="bg-white border border-gray-200 md:py-4 md:px-6 py-4 px-4 md:mb-6 mb-4">
                            <div className="flex justify-between">
                                <div className="md:w-full w-10/12">
                                    <h1 className="text-3xl font-medium text-black capitalize">{data.name.toLowerCase()}</h1>
                                    {!isCurrentMobile && (
                                        <div className="md:flex items-center my-4 space-x-20 font-normal hidden">
                                            <div className="flex items-center space-x-3">
                                                ...
                                                <span className="text-custom-primary text-lg">{data.total_courses} Course(s)</span>
                                            </div>
                                            <div className="flex items-center space-x-3">
                                                <FontAwesomeIcon icon={faUniversity} className="w-6" />{' '}
                                                <span className="text-custom-primary text-lg">{data.total_universities} Universitie(s)</span>
                                            </div>
                                        </div>
                                    )}
                                </div>
                            ...
                            {isCurrentMobile && (
                                <div className="flex items-center text-lg my-4 w-full justify-between font-normal md:hidden">
                                    <div className="flex items-center space-x-3">
                                        ...
                                        <span className="text-sm text-custom-primary">{data.total_courses} Course(s)</span>
                                    </div>
                                    <div className="flex items-center space-x-3">
                                        <FontAwesomeIcon icon={faUniversity} className="w-4" />{' '}
                                        <span className="text-sm text-custom-primary">{data.total_universities} Universitie(s)</span>
                                    </div>
                                </div>
                            )}
                            <div className="flex items-center md:space-x-8 w-full justify-between md:justify-start">
                                <div className="md:text-lg text-sm font-normal">
                                    <span className="font-medium">Academy year</span> : {data.academic_year}
                                </div>
                                {data.video && (
                                    <div>
                        ...
                        {isCurrentMobile && (
                            <div className="mb-4 bg-white">
                                {studyMobileLevel.map((item, i) => {
                                    return (
                                        <div className="grid grid-cols-3 divide-x divide-gray-200 border-b border-gray-200" key={i}>
                                            {item.map((subItem, i) => {
                                                return (
                                                    <div className="py-3 px-2 break-words items-center text-center text-xs" key={i}>
                                            ...
                                            {item.length % 3 !== 0 && <div className="py-3 px-2 break-words items-center text-center text-xs" />}
                                        </div>
                                    );
                                })}
                            </div>
                            ...
                            <div className="md:p-6 p-4">
                                <div className="flex items-center space-x-6">
                                    <div className="md:w-1/3 w-1/2">
                                        ...
                                        >
                                            <ItemDropdown title={'Select city/province'} value={''} classInactive="font-medium text-custom-primary">
                                            ...
                                            {city.map((item, i) => {
                                                return (
                                                    <ItemDropdown
                                                        title={item.name}
                                                        value={item.slug}
                                                        classInactive="text-custom-primary"
                                                        key={item.id}
                                                    >
                                                        {item.name}
                                                    </ItemDropdown>
                                    ...
                                    <div className="w-1/2 md:w-1/3">
                                        ...
                                        >
                                            <ItemDropdown title={'Select School Type'} value={''} classInactive="font-medium text-custom-primary">
                                                Select School Type
                                            </ItemDropdown>
                                ...
                                <div className="">
                                    {children.map((item, i) => {
                                        return (
                                            <div key={i}>
                                                <div className="md:text-2xl text-xl font-normal text-black mt-5 md:mt-12 mb-3">{item.name}</div>
                                                <div className="md:pl-5 md:text-lg text-custom-primary font-normal">
                                                    <ul className="space-y-2">
                                                        {item.universities.nodes.map((subItem, i) => {
                                                            return (
                                                                <li key={i}>
                                                                    <Link href={`/university/${subItem.slug}`}>
                                                                        <a>{subItem.title}</a>
                                                                    </Link>{' '}
                                                                    <span className="text-gray-400 text-xs md:text-base ml-2 md:ml-5 font-thin italic">
                                                                        ({subItem.course_count} Course(s))
                                                                    </span>
                                                                </li>
                                                            );
                                                        })}
                        ...
                        {!isCurrentMobile && (
                            <div className="bg-white border border-gray-200 mb-6 text-black text-sm leading-6">
                                {studyLevel.map((item, i) => {
                                    return (
                                        <div className="grid grid-cols-2 divide-x divide-gray-200 border-b border-gray-200" key={i}>
                                            {item.map((subItem, i) => {
                                                return (
                                                    <div className="p-4 flex flex-col items-center text-center" key={i}>
                                            ...
                                            {item.length % 2 !== 0 && <div className="p-4 flex flex-col items-center text-center" />}
                                        </div>
                                    );
                                })}
                            </div>
                            ...
                            <div>
                                <Accordion className={styles.accordion} allowZeroExpanded>
                                    {categoryCourse.map((item, i) => {
                                        return (
                                            <AccordionItem
                                                activeClassName="bg-custom-primary bg-opacity-10 text-black font-normal"
                                                uuid={item.id}
                                                key={i}
                                            >
                                                <AccordionItemHeading>
                                                            ...
                                                            <AccordionItemState>
                                                                {({ expanded }) =>
                                                                    expanded ? (
                                                                        <FontAwesomeIcon
                                                                            icon={faChevronDown}
                                                                            className="w-4 h-4 text-base text-gray-600"
                                                                        />
                                                                    ) : (
                                                                        <FontAwesomeIcon
                                                                            icon={faChevronLeft}
                                                                            className="w-4 h-4 text-base text-custom-secondary"
                                                                        />
                                                                    )
                                                                }
                                                            </AccordionItemState>
