          ...
          <div className="md:col-span-2">
                  ...
                  {!isCurrentMobile && (
                    <div className="md:flex items-center my-4 space-x-20 font-normal hidden">
                      <div className="flex items-center space-x-3">
                        ...
                        <span className="text-custom-primary text-lg">
                          {data.total_courses} Course(s)
                        </span>
                      </div>
                      <div className="flex items-center space-x-3">
                        <FontAwesomeIcon icon={faUniversity} className="w-6" />{' '}
                        <span className="text-custom-primary text-lg">
                          {data.total_universities} Universitie(s)
                        </span>
                      </div>
                    </div>
                  )}
            ...
            {isCurrentMobile && (
              <div className="mb-4 bg-white">
                {studyMobileLevel.map((item, i) => {
                  return (
                    <div
                      className="grid grid-cols-3 divide-x divide-gray-200 border-b border-gray-200"
                      key={i}
                    >
                      {item.map((subItem, i) => {
                        return (
                          <div
                            className="py-3 px-2 break-words items-center text-center text-xs"
                            key={i}
                          >
                            <div className="break-words inline text-custom-primary font-normal">
                              {subItem.name}
                            </div>
            ...
            <div className="bg-white border border-gray-200 mb-6">
              <div className="md:p-4 py-2 px-4 font-normal text-2xl border-b border-gray-200 text-black text-custom-secondary">
                University in <span className="capitalize">{data.name.toLowerCase()}</span>{' '}
                <span>({data.total_universities})</span>
              </div>
            ...
            {!isCurrentMobile && (
              <div className="bg-white border border-gray-200 mb-6 text-black text-sm leading-6">
                {studyLevel.map((item, i) => {
                  return (
                    <div
                      className="grid grid-cols-2 divide-x divide-gray-200 border-b border-gray-200"
                      key={i}
                    >
                      {item.map((subItem, i) => {
                        return (
